...
class RSM_SS(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.dims = dims
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule  linspace均分
        ...
        self.patch_embed = _make_patch_embed(in_chans, dims[0], patch_size, patch_norm, norm_layer)  # 初始化，输入通道、维度，补丁大小、是否正否归一化，什么类型的归一化（上面刚定义好的）
        ...
        self.conv_out_seg = nn.Conv2d(self.dims[0]//4, self.num_classes, kernel_size=7, stride=1, padding=3)  # 这个地方是模型的最后一层，默认输出是1层，但是我们要改成多分类
    ...
    def forward(self, x1: torch.Tensor):
        x1 = self.patch_embed(x1) # 8,3,256,256 --> 8,64,64,96

        x1_1 = self.encoder_block1(x1) # 8,64,64,96 --> 8,64,64,96
        x1_2 = self.encoder_block2(x1_1) # 8,64,64,96 --> 8,32,32,192
        x1_3 = self.encoder_block3(x1_2) # 8,32,32,192 --> 8,16,16,384
        x1_4 = self.encoder_block4(x1_3)  # b,h,w,c  8,16,16,384 --> 8,8,8,768

        x1_1 = rearrange(x1_1, "b h w c -> b c h w").contiguous() # 8,64,64,96 --> 8,96,64,64
        x1_2 = rearrange(x1_2, "b h w c -> b c h w").contiguous() # 8,32,32,192 --> 8,192,32,32
        x1_3 = rearrange(x1_3, "b h w c -> b c h w").contiguous() # 8,16,16,384 --> 8,384,16,16
        x1_4 = rearrange(x1_4, "b h w c -> b c h w").contiguous() # 8,8,8,768 --> 8,768,8,8


        decode_3 = self.deocder_block3(x1_4, x1_3) # 8,768,8,8   8,384,16,16 --> 8,384,16,16
        decode_2 = self.deocder_block2(decode_3, x1_2) # 8,384,16,16   8,192,32,32  --> 8,192,32,32
        decode_1 = self.deocder_block1(decode_2, x1_1) # 8,192,32,32   8,96,64,64  --> 8,96,64,64

        output = self.upsample_x4(decode_1) # 8,96,64,64  --> 8,8,256,256
        output = self.conv_out_seg(output) # 8,8,256,256 --> 8,1,256,256
