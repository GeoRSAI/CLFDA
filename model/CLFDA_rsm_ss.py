...
class OSSM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        x = self.act(x)
        # y = self.forward_core(x, channel_first=with_dconv)  #   这里进行了修改 ，加了v2
        y = self.forward_corev2(x, channel_first=with_dconv)  # 这里进行了修改 ，加了v2
        if not self.disable_z:
            y = y * z
...
class FD(nn.Module):
    """FD."""
    def __init__(self, in_channel,act_layer=nn.GELU,wave='haar'):
        super().__init__()
        self.dwt = DWTForward(J=1,wave=wave,mode='reflect')
        self.channel_attention = ChannelAtt(gate_channels=in_channel*3, reduction_ratio=2, pool_types=['avg'])
        self.c_conv = ConvBN(in_channel*3, in_channel, kernel_size=1)
        self.c_act = act_layer()
        # self.mask_attention = FusionAttention(dim=in_channel, ssmdims=in_channel, num_heads=16, qkv_bias=False,window_size=8)
        self.mask_attention = GlobalLocalAttention(dim=in_channel, num_heads=16, qkv_bias=False,window_size=8)
        self.ln1 = nn.LayerNorm(in_channel, eps=1e-6)
        self.ln2 = nn.LayerNorm(in_channel*3, eps=1e-6)

    def forward(self, ILL):  # -> 8,128,128,96
        ILL = ILL.permute(0, 3, 1, 2).contiguous() # -> 8,96,128,128
        ILL, Yh = self.dwt(ILL) # 8,96,64,64 -> 8,96,3,64,64
        ILL = ILL.permute(0, 2, 3, 1).contiguous()
        ILL = self.ln1(ILL)
        ILL = ILL.permute(0, 3, 1, 2).contiguous()  # 8,96,64,64
        FLL = self.mask_attention(ILL)  # 8,96,64,64

        Fh = rearrange(Yh[0], 'b c t h w -> b (c t) h w')# 8,96,3,64,64  -> 8,288,64,64
        Fh = Fh.permute(0, 2, 3, 1).contiguous()   # 8,64,64,288
        Fh = self.ln2(Fh)
        Fh = Fh.permute(0, 3, 1, 2).contiguous()   # 8,288,64,64
        Fh = self.channel_attention(Fh) # 对应下面Fc_att  # 8,288,64,64
        Fh = self.c_conv(Fh) # 8,96,64,64
        Fh = self.c_act(Fh) # 8,96,64,64
        # return ILL.permute(0, 2, 3, 1).contiguous(), FLL.permute(0, 2, 3, 1).contiguous(),  Fh.permute(0, 2, 3, 1).contiguous()
        return ILL.permute(0, 2, 3, 1).contiguous(), FLL.permute(0, 2, 3, 1).contiguous(),  Fh

class FD_second(nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, Fr, Fc_att):
        Fr = torch.cat((Fr, Fc_att), dim=1)
        return Fr


class RSM_SS_OURS(nn.Module):
    def __init__(
            self,
            patch_size=4,
            in_chans=3,
        ...
        self.dims = dims
        # =============
        self.add_merging_dims = [0, 0, dims[0]*2,dims[0]*6]
        self.add_encoder_dims = [0,dims[0],dims[0]*3,dims[0]*7]
        self.add_decoder_dims = [0,dims[0],dims[0],dims[0]]
        # =============
        dpr = [x.item() for x in
               torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule  linspace均分
        ...
        if isinstance(norm_layer, str) and norm_layer.lower() in ["ln"]:  # 根据传入的参数确定后面使用的LN
            norm_layer: nn.Module = _NORMLAYERS[norm_layer.lower()]

        if isinstance(ssm_act_layer, str) and ssm_act_layer.lower() in ["silu", "gelu", "relu"]:  # 根据传入的参数确定后面使用的激活函数
            ssm_act_layer: nn.Module = _ACTLAYERS[ssm_act_layer.lower()]

        if isinstance(mlp_act_layer, str) and mlp_act_layer.lower() in ["silu", "gelu", "relu"]:  # 根据传入的参数确定后面使用的激活函数
            mlp_act_layer: nn.Module = _ACTLAYERS[mlp_act_layer.lower()]

        _make_patch_embed = self._make_patch_embed_v2  # 定义编码器最开始的补丁嵌入
        self.patch_embed = _make_patch_embed(in_chans, dims[0], patch_size, patch_norm,
                                             norm_layer)  # 初始化，输入通道、维度，补丁大小、是否正否归一化，什么类型的归一化（上面刚定义好的）

        _make_downsample = self._make_downsample_v3  # 定义下采样的过程，变形——卷积——变形

        # self.encoder_layers = [nn.ModuleList()] * self.num_layers
        self.encoder_layers = []  # 开始定义编码层
        self.decoder_layers = []  # 开始定义解码器
        self.downsample = []

        # 将下采样拆出来 =================
        for i_layer in range(self.num_layers):  # [2, 2, 9, 2]   一共是4层
            self.downsample.append(_make_downsample(
                self.dims[i_layer - 1]+self.add_encoder_dims[i_layer-1], # =======================
                self.dims[i_layer]+self.add_merging_dims[i_layer], # ============================
        ...
        for i_layer in range(self.num_layers):  # [2, 2, 9, 2]   一共是4层
            ...
            downsample = nn.Identity() # 下采样已经被拆分
            # downsample = _make_downsample(
            #     self.dims[i_layer - 1]+self.add_encoder_dims[i_layer-1], # =======================
            #     self.dims[i_layer]+self.add_merging_dims[i_layer], # ============================
            #     norm_layer=norm_layer,
            ...
            self.encoder_layers.append(self._make_layer(
                dim=self.dims[i_layer]+self.add_encoder_dims[i_layer], # ===========================
            ...
            if i_layer != 0:
                self.decoder_layers.append(
                    Decoder_Block(in_channel=self.dims[i_layer]+self.add_encoder_dims[i_layer]+self.add_decoder_dims[i_layer], out_channel=self.dims[i_layer - 1]+self.add_encoder_dims[i_layer-1]+self.add_decoder_dims[i_layer-1]))

        self.encoder_block1, self.encoder_block2, self.encoder_block3, self.encoder_block4 = self.encoder_layers
        self.down_block1, self.down_block2, self.down_block3, self.down_block4 = self.downsample
        # ===========================================        # 这里边添加SFF
        # self.sff1 = FD(in_channel=self.dims[0],act_layer=ssm_act_layer)
        self.sff2 = FD(in_channel=self.dims[0],act_layer=ssm_act_layer)
        self.sff3 = FD(in_channel=self.dims[0],act_layer=ssm_act_layer)
        self.sff4 = FD(in_channel=self.dims[0],act_layer=ssm_act_layer)
        # self.sff_second1 = FD_second()
        self.sff_second2 = FD_second()
        self.sff_second3 = FD_second()
        self.sff_second4 = FD_second()
        # ===========================================
        self.deocder_block1, self.deocder_block2, self.deocder_block3 = self.decoder_layers  # 解码器设置三次，后面应该需要再有一次上采样 self.upsample_x4
        ...
        self.upsample_x4 = nn.Sequential(
            ...
            nn.Conv2d(self.dims[0] // 2, self.dims[0] // 4, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(self.dims[0] // 4),
            nn.ReLU(inplace=True),
            nn.UpsamplingBilinear2d(scale_factor=2)
        )
        self.conv_out_seg = nn.Conv2d(self.dims[0] // 4, self.num_classes, kernel_size=7, stride=1,
                                      padding=3)  # 这个地方是模型的最后一层，默认输出是1层，但是我们要改成多分类

        self.apply(self._init_weights)  # 给线性层和正则化化层初始化权重信息
    ...
    def forward(self, x1: torch.Tensor):
        x1 = self.patch_embed(x1)  # 4,3,512,512 --> 4,128,128,96
        x1_1 = self.down_block1(x1) # 4,128,128,96 --> 4,128,128,96
        x1_1 = self.encoder_block1(x1_1)  # 4,128,128,96 --> 4,128,128,96

        x1_2 = self.down_block2(x1_1) # 4,128,128,96 --> 8,64,64,192
        ILL, FLL,  Fh2 = self.sff2(x1) # 8,64,64,96 --> 8,64,64,96;   8,64,64,96;   8,64,64,96
        x1_2 = torch.cat([x1_2, FLL], dim=-1) ## 8,64,64,192 --- 8,64,64,96 ->  8,64,64,288
        x1_2 = self.encoder_block2(x1_2)  # 8,64,64,288 --> 8,64,64,288

        x1_3 = self.down_block3(x1_2) # 8,64,64,288 --> 8,32,32,576
        ILL, FLL,  Fh3 = self.sff3(ILL)  # 8,64,64,96 --> 8,32,32,96;   8,32,32,96;   8,32,32,96
        x1_3 = torch.cat([x1_3, FLL], dim=-1)  # 8,32,32,576;8,32,32,96 -->8,32,32,672
        x1_3 = self.encoder_block3(x1_3)  # 8,32,32,672 --> 8,32,32,672

        x1_4 = self.down_block4(x1_3) # 8,32,32,672 --> 8,16,16,1344
        ILL, FLL, Fh4 = self.sff4(ILL)# 8,32,32,96 --> 8,16,16,96;   8,16,16,96;   8,16,16,96
        x1_4 = torch.cat([x1_4, FLL], dim=-1) # 8,16,16,1440;
        x1_4 = self.encoder_block4(x1_4)  # b,h,w,c  8,16,16,384 --> 8,8,8,768


        x1_1 = rearrange(x1_1, "b h w c -> b c h w").contiguous()  # 8,128,128,96 --> 8,96,128,128
        x1_2 = rearrange(x1_2, "b h w c -> b c h w").contiguous()  # 8,64,64,288 --> 8,288,64,64
        x1_3 = rearrange(x1_3, "b h w c -> b c h w").contiguous()  # 8,32,32,672 --> 8,672,32,32
        x1_4 = rearrange(x1_4, "b h w c -> b c h w").contiguous()  # 8,16,16,1440 --> 8,1440,16,16

        x1_2 = self.sff_second2(x1_2,Fh2) # 8,384,64,64
        x1_3 = self.sff_second3(x1_3,Fh3) # 8,768,32,32
        x1_4 = self.sff_second4(x1_4,Fh4) # 8,1536,16,16


        decode_3 = self.deocder_block3(x1_4, x1_3)  # 8,1536,16,16   8,768,32,32 --> 8,768,32,32
        decode_2 = self.deocder_block2(decode_3, x1_2)  # 8,768,32,32   8,384,64,64  --> 8,384,64,64
        decode_1 = self.deocder_block1(decode_2, x1_1)  # 8,384,64,64   8,96,128,128  --> 8,96,128,128

        output = self.upsample_x4(decode_1)  # 8,96,128,128  --> 8,24,512,512
        output = self.conv_out_seg(output)  # 8,24,512,512 --> 8,16,512,512
