    ...
    def forward(self, x):
     ...
     #   pool10 = self.maxpool(conv4_3)
        ...
        if self.add_output:
            ...
            x_out2 = [torch.sigmoid(r) for r in results]  # edge
            x_out3 = torch.sigmoid(x_out3)

        return [x_out1, x_out2, x_out3]  # mask, edge
...
def seanet(num_classes=1, num_channels=3, use_bn=False):
    model = SEANet(num_classes=num_classes)
    pretained_path = r"D:\xjwdeeplearning\202407pesudosamples\exp006_seanet\vgg16convs.mat"
    load_vgg16pretrain(model, pretained_path)
    return model
